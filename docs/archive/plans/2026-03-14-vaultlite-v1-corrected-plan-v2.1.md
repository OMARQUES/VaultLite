...
# VaultLite V1 Corrected Foundation Plan v2.1
...
- password rotation
- import/export
- minimal owner/admin user lifecycle operations: suspend, reactivate, deprovision/delete
...
- `active`
- `suspended`
- `deprovisioned` (or `deleted`, if that term is chosen consistently in the ADR)
...
- **deprovisioned/deleted:** revokes active sessions, invalidates trusted device access, removes normal user access permanently, and triggers retention/cleanup policy defined by operations docs
...
5. Owner/admin may deprovision/delete an account according to the operations policy.
6. Deprovision/delete revokes sessions, invalidates trusted-device access, and triggers retention/cleanup rules for encrypted user data and blobs.
