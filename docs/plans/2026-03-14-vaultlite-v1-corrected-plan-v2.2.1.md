# VaultLite V1 Corrected Foundation Plan v2.2.1

## Purpose
This document supersedes the previous corrected foundation plan v2.2 and incorporates the remaining contract-hardening issues identified during review.
...
### Phase 7.5 - UX/UI Visual Baseline
This phase exists to avoid two failure modes:
- designing too early, before auth and core vault workflows are real
- designing too late, after attachments, sync, admin flows, and extension surfaces create unnecessary rework

The UX/UI visual baseline should start only after the Phase 7 core data and interaction model is stable enough to avoid churn, and should land before Phase 8 and beyond multiply UI surfaces.

Scope of this phase:
1. define the visual direction for the web app core: typography, spacing, color tokens, surface hierarchy, and motion rules
2. define shared interaction patterns for auth, onboarding, vault list, item editor, error states, empty states, and destructive actions
3. apply the approved baseline to the current web core so later phases inherit the same patterns instead of inventing local one-off UI decisions
4. run a responsive and accessibility pass on the stabilized web core before attachments, sync, and owner/admin operational UI expand the surface area

Out of scope for this phase:
- extension-specific UI polish
- advanced marketing or landing-page design work
- speculative design-system abstraction beyond what the current web core actually needs
- major product-flow changes that belong to earlier functional phases

The goal is not to make the app “pretty” in isolation. The goal is to freeze a coherent visual and interaction baseline at the point where the product is already functionally real, but before future features amplify inconsistency and retrofit cost.
...
## Summary of Changes vs Previous Corrected Plan v2.2
