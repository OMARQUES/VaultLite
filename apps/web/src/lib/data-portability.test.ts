...
function createSessionStoreStub(username = 'alice'): SessionStore {
  return {
    ...
    getIconsState: vi.fn(async () => ({
      status: 'not_modified' as const,
      etag: null,
    })),
    putIconDomainsBatch: vi.fn(async () => ({
      ok: true as const,
      acceptedItems: 0,
      entries: [],
      updatedAt: new Date().toISOString(),
    })),
    putIconDomainsItem: vi.fn(async () => ({
      ok: true as const,
      result: 'success_no_op' as const,
      domainsChanged: false,
      itemId: 'item_stub',
      itemRevision: 0,
      updatedAt: new Date().toISOString(),
    })),
    startIconDomainsReindex: vi.fn(async () => ({
      ok: true as const,
      result: 'success_no_op' as const,
    ...
    sendIconDomainsReindexChunk: vi.fn(async () => ({
      ok: true as const,
      result: 'success_no_op' as const,
      generationId: 'gen_stub',
      acceptedItems: 0,
    ...
    commitIconDomainsReindex: vi.fn(async () => ({
      ok: true as const,
      result: 'success_no_op' as const,
      generationId: 'gen_stub',
      acceptedItems: 0,
    ...
    issueIconObjectTickets: vi.fn(async () => ({
      ok: true as const,
    ...
    upsertPasswordGeneratorHistoryEntry: vi.fn(async () => ({
      ok: true as const,
      result: 'success_changed' as const,
    })),
