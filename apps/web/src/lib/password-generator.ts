...
export type PasswordGeneratorMode = (typeof PASSWORD_GENERATOR_MODES)[keyof typeof PASSWORD_GENERATOR_MODES];
...
const DEFAULT_STATE: PasswordGeneratorState = Object.freeze({
...
function normalizeMode(mode: unknown): PasswordGeneratorMode {
...
function chooseCharacter(pool: string): string {
  if (!pool) {
    return '';
  }
...
function buildSmartPassword(length: number): string {
...
function buildRandomPassword(length: number, includeNumbers: boolean, includeSymbols: boolean): string {
...
function buildPinCode(length: number): string {
  const chars: string[] = [];
  for (let index = 0; index < length; index += 1) {
...
export function createDefaultGeneratorState(): PasswordGeneratorState {
  return { ...DEFAULT_STATE };
}

export function normalizeGeneratorState(input: Partial<PasswordGeneratorState> | null | undefined): PasswordGeneratorState {
...
export function generatePassword(input: Partial<PasswordGeneratorState> | null | undefined): string {
