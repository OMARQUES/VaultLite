...
export function addGeneratorHistoryEntry(entries, nextEntry, maxEntries = 60) {
...
export function filterGeneratorHistoryEntries(entries, query) {
...
export function groupGeneratorHistoryByDay(entries) {
  const normalizedEntries = Array.isArray(entries) ? entries : [];
  const groups = [];
  const byDay = new Map();
